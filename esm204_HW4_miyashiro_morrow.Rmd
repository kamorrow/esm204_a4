...
df_2b <- data.frame(increase = c("none", "T", "g", "eta", "beta")) %>%
  mutate(T = as.numeric(c(4.4, 4.84, 4.4, 4.4, 4.4)),
         g = as.numeric(c(g, g, g * 1.1, g, g)),
         eta = as.numeric(c(eta, eta, eta, eta * 1.1, eta)),
         beta = as.numeric(c(beta, beta, beta, beta, beta * 1.1))) %>%
  mutate(sum = sum(T))
